...
function classifyProgram(p) {
  ...
  const kw = Array.isArray(p.keywords) ? p.keywords.join(' ').toLowerCase() : (p.keywords || '').toLowerCase();
  const desc = (p.description || '').toLowerCase();
  const allText = name + ' ' + kw + ' ' + desc;
  const id = (p.id || '').toLowerCase();

  // Retail discounts
  if (allText.includes('discount') || allText.includes('% off') ||
      allText.includes('verizon') || allText.includes('t-mobile') || allText.includes('firstnet') ||
      allText.includes('home depot') || allText.includes('lowes') ||
      allText.includes('nfl shop') || allText.includes('mlb shop') ||
      allText.includes('nba store') || allText.includes('nhl shop') ||
      allText.includes('under armour') || allText.includes('govx') ||
      allText.includes('outback steakhouse') ||
      (allText.includes('nike') && allText.includes('10%')) ||
      (allText.includes('student') && (allText.includes('spotify') ||
       allText.includes('apple music') || allText.includes('amazon prime') ||
       allText.includes('hulu') || allText.includes('adobe') || allText.includes('youtube')))) {
    return 'retail';
  }

  // Veterans (excluding LGBTQ+ veteran services)
  if ((allText.includes('calvet') ||
       (allText.includes('veteran') && !allText.includes('lgbtq'))) &&
  ...
  // LGBTQ services
  if (allText.includes('lgbtq') || allText.includes('lgbt center') ||
      allText.includes('gay history') || allText.includes('lesbian') ||
      (allText.includes('trans') && !allText.includes('transition') &&
       !allText.includes('transportation') && !allText.includes('transit')) ||
      allText.includes('queer') || allText.includes('pflag') ||
      id.includes('lgbtq') || id.includes('lgbt') ||
      id.includes('trans') && !id.includes('transit')) {
    return 'lgbtq';
  }

  // Domestic violence / Safety services
  if (allText.includes('domestic violence') || allText.includes('abuse') ||
      allText.includes('relationship abuse') || allText.includes('love is respect') ||
      allText.includes('shelter') && allText.includes('women') ||
      name.includes('safe place') || name.includes('safe haven') ||
      id.includes('cora') || id.includes('stand') ||
      id.includes('w-o-m-a-n') || id.includes('woman-inc') ||
      id.includes('la-casa') || id.includes('building-futures') ||
      id.includes('next-door') || id.includes('narika') || id.includes('maitri') ||
      id.includes('asian-women') || id.includes('family-justice')) {
    return 'safety';
  }

  // Reentry services
  if (allText.includes('reentry') || allText.includes('incarcerat') ||
      allText.includes('expunge') || allText.includes('recidiv') ||
      allText.includes('formerly incarcerated')) {
    return 'legal';
  }

  // Childcare / Early education
  if (allText.includes('childcare') || allText.includes('child care') ||
      allText.includes('preschool') || allText.includes('head start') ||
      allText.includes('crisis nursery') && allText.includes('emergency childcare')) {
    return 'education';
  }

  // Employment services
  if ((allText.includes('job training') || allText.includes('workforce') ||
       allText.includes('employment') || allText.includes('career')) &&
      !allText.includes('veteran')) {
    if (id.includes('jobsnow') || id.includes('job-forum') ||
        id.includes('citybuild') || id.includes('techsf') ||
  ...
  if (allText.includes('immigration') || allText.includes('immigrant') ||
      allText.includes('citizenship') || allText.includes('daca')) {
    return 'legal';
  }

  // Housing services (should be in housing)
  if (id.includes('housing') || id.includes('homeless') ||
      (allText.includes('housing') && allText.includes('program'))) {
    // Only move if it's clearly a housing program, not just mentions housing
    if (id === 'hip-housing-home-sharing' ||
        id === 'echo-housing' ||
...
function reorganize() {
  ...
  writeYaml('community.yml', newCommunity,
  ...
  if (buckets['retail'].length > 0) {
    ...
    writeYaml('retail.yml', allRetail,
  ...
  if (buckets['federal-benefits'].length > 0) {
    ...
    writeYaml('federal-benefits.yml', allFederal,
  ...
  if (buckets['education'].length > 0) {
    ...
    writeYaml('education.yml', allEducation,
  ...
  if (buckets['employment'].length > 0) {
    ...
    writeYaml('employment.yml', allEmployment,
  ...
  if (buckets['housing'].length > 0) {
    ...
    writeYaml('housing.yml', allHousing,
